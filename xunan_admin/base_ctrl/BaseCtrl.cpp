// D:\cppwork\myproj\xunan_admin\ctrl\BaseCtrl.cpp : 实现文件
//
...
IMPLEMENT_DYNAMIC(BaseCtrl, CListCtrl)

BaseCtrl::BaseCtrl()
{
	for (int i = 0; i < MAX_LISTITEM; i++){
		this->m_bItemCheckState[i] = FALSE;
	}
}
...
void BaseCtrl::OnDbClick(NMHDR*pNMHDR, LRESULT*pResult){
	std::cout << "" << std::endl;
...
int BaseCtrl::GetFirsetSelectedItem()
{
	int nItem = -1;
...
int BaseCtrl::GetNextSelectedItem(int nItem)
{
	int nNextItem = -1;
...
BOOL BaseCtrl::GetCheckBoxItemState(int nItem)
{
	return m_bItemCheckState[nItem];
}

void BaseCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
BOOL BaseCtrl::SetCheckBoxItemState(int nItem, BOOL bNewState)
{
...
void BaseCtrl::OnItemChanged(NMHDR*pNMHDR, LRESULT*pResult){
	//int nItem = this->GetNextItem(-1, LVNI_SELECTED);
	//CString id = this->GetItemText(nItem, 0);
	//CString name = this->GetItemText(nItem, 1);
	//CString info = this->GetItemText(nItem, 2);
		...
		)
	{
		...
		this->SetCheckBoxItemState(pNMLV->iItem, TRUE);
	}
...
BEGIN_MESSAGE_MAP(BaseCtrl, CListCtrl)
